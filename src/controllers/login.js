...
export default async function login(req, res) {
  ...
  try {
    ...
    const session = await connection.query(
      'SELECT * FROM session WHERE customer_id = $1;',
      [customer.id],
    );

    if (session.rows.length === 0) {
      await connection.query(
        'INSERT INTO session (customer_id,token) VALUES ($1,$2);',
        [customer.id, token],
      );
    } else {
      await connection.query(
        'UPDATE session SET token = $1 WHERE customer_id = $2;',
        [token, customer.id],
      );
    }

    const { name } = emailExistsCheck.rows[0];
    const { id } = emailExistsCheck.rows[0];
